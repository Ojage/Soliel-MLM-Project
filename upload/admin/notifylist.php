...
if (isset($ntId) && intval($ntId) > 0) {
    ...
    $cctoadminarr = array(0, 1);
    $cctoadmin_cek = radiobox_opt($cctoadminarr, $ntoptionsarr['cctoadmin']);
...
$msgListData = $db->getRecFrmQry("SELECT * FROM " . DB_TBLPREFIX . "_notifytpl WHERE 1 GROUP BY ntcode ORDER BY ntid ASC");
...
if (isset($FORM['dosubmit']) and $FORM['dosubmit'] == '1') {
    ...
    $ntoptions = put_optionvals($ntoptions, 'cctoadmin', $cctoadmin);
                            ...
                            <select name="ntId" class="form-control select1">
                                ...
                                if (count($msgListData) > 0) {
                                    foreach ($msgListData as $val) {
                                        $emailopt = get_optionvals($val['ntoptions'], 'email');
                                        $emailoptstr = ($emailopt == '1') ? " &#10003;" : "";
                                        $strsel = ($msgtplrow['ntid'] == $val['ntid']) ? ' selected' : '';
                                        echo "<option value='{$val['ntid']}'{$strsel}>" . $val['ntname'] . $emailoptstr . "</option>";
                                    }
                                } else {
                ...
                <form method="post" action="index.php" id="msgtplform">
                    ...
                    <div class="card-body">
                        ...
                        if ($msgtplrow['ntid'] > 0) {
                            ...
                            <div class="form-group">
                                <label for="selectgroup-pills">Copy Email to Admin</label>
                                <div class="selectgroup selectgroup-pills">
                                    <label class="selectgroup-item">
                                        <input type="radio" name="cctoadmin" value="0" class="selectgroup-input"<?php echo myvalidate($cctoadmin_cek[0]); ?>>
                                        <span class="selectgroup-button selectgroup-button-icon"><i class="fas fa-fw fa-times-circle"></i> No</span>
                                    </label>
                                    <label class="selectgroup-item">
                                        <input type="radio" name="cctoadmin" value="1" class="selectgroup-input"<?php echo myvalidate($cctoadmin_cek[1]); ?>>

...
$filetypenow = 'file';
$avaldlfileopt_array = array(0 => "Public", 1 => "Registered Member", 2 => "Registered Member with Membership Active", 3 => "Registered Member with Membership Inactive");
$flavalto_menu = select_opt($avaldlfileopt_array);
...
if (isset($editId) and $editId != "") {
    ...
    $flavalto_menu = select_opt($avaldlfileopt_array, $rowstr['flavalto']);
...
if (isset($FORM['dosubmit']) and $FORM['dosubmit'] == '1') {
    ...
    $flppids = (is_array($flppids)) ? '"' . implode('","', $flppids) . '"' : '';
    ...
    <div class="col-md-12">
        ...
        <form method="post" action="doupfile.php" enctype="multipart/form-data">
            ...
            foreach ($row as $value) {
                ...
                $strsel = ($grouprow['grid'] == $rowstr['flgrid']) ? ' selected' : '';
                $groupcatlist .= "<option value='{$grouprow['grid']}'{$strsel}>{$grouprow['grtitle']}</option>";
            }

            $flppidsarr = str_getcsv($rowstr['flppids']);
            $flppids_menu = ppdblist($flppidsarr);
            ?>
            <div class="form-row">
                <div class="form-group col-md-5">
                    <label>Category</label>
                    <select name="flgrid" id="flgrid" class="form-control" data-height="100%">
                        <?php echo myvalidate($groupcatlist); ?>
                    </select>
                </div>
                <div class="form-group col-md-7">
                    <label>Available Payplan</label>
                    <select name="flppids[]" id="flppids" class="form-control" multiple="multiple" data-height="100%">
                        <?php echo myvalidate($flppids_menu); ?>
                    </select>
                </div>
            </div>

...
$avalpaymentoptarr = array();
foreach ($avalpaymentopt_array as $key => $value) {
    if ($key == 'manualpayipn') {
        $value = $payrow[$value];
    }
    $avalpaymentoptarr[$key] = $value;
}
$txpaytype_menu = select_opt($avalpaymentoptarr);
...
if (isset($editId) and $editId != "") {
    ...
    $txpaytype_menu = select_opt($avalpaymentoptarr, $rowstr['txpaytype']);

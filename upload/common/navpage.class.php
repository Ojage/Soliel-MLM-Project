...
class Paginator {
    ...
    function paginate() {
        ...
        if ($this->num_pages > 1) {
            ...
            $this->return .= ($_GET['page'] == 'All') ? "<li class='page-item active'><a class=\"page-link\" hidden href=\"javascript:;\">{$LANG['g_all']}</a></li>" : "<li class='page-item'><a class=\"page-link\" hidden href=\"$_SERVER[PHP_SELF]?page=1&ipp=All$this->querystring\">{$LANG['g_all']}</a></li></ul>";
        } else {
            for ($i = 1; $i <= $this->num_pages; $i++) {
            ...
            $this->return .= "<li class='page-item'><a class=\"page-link\" href=\"$_SERVER[PHP_SELF]?page=1&ipp=All$this->querystring\">{$LANG['g_all']}</a></li></ul>";
        }
        $this->low = ($this->current_page <= 0) ? 0 : ((int) $this->current_page - 1) * (int) $this->items_per_page;
    ...
    function display_items_per_page() {
        ...
        return "<span class=\"text-muted\">{$LANG['g_show']}:</span> <select class=\"border form-control-sm text-muted\" onchange=\"window.location='$_SERVER[PHP_SELF]?page=1&ipp='+this[this.selectedIndex].value+'$this->querystring';return false\">$items</select>";
    }
    ...
    function display_jump_menu() {
        ...
        return "<span class=\"text-muted\">{$LANG['g_page']}:</span> <select class=\"border form-control-sm text-muted\" onchange=\"window.location='$_SERVER[PHP_SELF]?page='+this[this.selectedIndex].value+'&ipp=$this->items_per_page$this->querystring';return false\">$option</select>";
    }

    function items_total() {
        global $LANG;
        return "{$LANG['g_total']}: " . $this->items_total;
    }
